...
cache = joblib.load("file_cache.pkl")
    ...
    if is_file_in_folder(folder_path):
        ...
        for file in os.listdir(folder_path):
            ...
            if not file in cache:
                filename = file
                cache.add(filename)
                joblib.dump(cache, "file_cache.pkl")
                ...
                if is_anomaly:
                    save_in_folder("anomalies", filename, file_path, remove_file)
                    to_owl(KNN_class, filename, kupa_nr)
                else:
                    save_in_folder(KNN_class[0], filename, file_path, remove_file)
